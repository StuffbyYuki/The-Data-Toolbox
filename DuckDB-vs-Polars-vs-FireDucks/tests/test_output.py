...
from duckdb_vs_polars_vs_fireducks.queries.groupby_agg_fireducks import groupby_agg_fireducks
...
from duckdb_vs_polars_vs_fireducks.queries.window_func_fireducks import window_func_fireducks
...
def test_data():
    ...
    df = pl.scan_csv(f"data/{DATASET_NAME}.csv", schema=POLARS_DTYPES).head(1000).collect()
...
def test_agg(test_data):
    ...
    fireducks_result = agg_fireducks(test_data)

    assert_frame_equal(polars_result, pl.from_arrow(duckdb_result), 
                      check_row_order=False, check_dtypes=False)
    assert_frame_equal(polars_result, pl.from_arrow(fireducks_result), 
                      check_row_order=False, check_dtypes=False)
...
def test_groupby_agg(test_data):
    ...
    fireducks_result = groupby_agg_fireducks(test_data)
