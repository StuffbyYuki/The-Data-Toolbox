...
def test_agg(df):
    ...
    assert_frame_equal(polars_df, pl.from_pandas(pandas_df), check_row_order=False, check_dtypes=False)
...
def test_window_func(df):
    ...
    pandas_df = pd.DataFrame(
        {
            "avg_fare_per_vendor": df.to_pandas().groupby("VendorID")["fare_amount"].transform(
                "mean"
            ),
            "ttl_amt_rank_per_pay_type": df.to_pandas().groupby("payment_type")[
                "total_amount"
